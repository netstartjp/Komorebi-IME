...
impl Backend {
    /// Create a backend from app-managed files.
    ///
    /// Android downloads models only after an explicit user action and verifies them before this
    /// constructor is reached, so the engine must not invoke its desktop Hugging Face cache.
    pub fn from_files(
        gguf_path: impl Into<String>,
        tokenizer_json_path: impl Into<String>,
        display_name: impl Into<String>,
    ) -> Self {
        Backend {
            gguf_path: gguf_path.into(),
            tokenizer_json_path: tokenizer_json_path.into(),
