...
を維持します。端末が通知するナビゲーションバー、必須ジェスチャー、キーボード切替などの
OS操作領域は下部セーフエリアとして保持し、背景は連続させたままキーだけを上へ退避します。
...
| 中 | Karukan変換のMozc学習連携 | Karukanで確定した変換をMozcが学習し、次回からMozc単体でも候補に出る |
| 中 | スペースキー長押しのトラックパッドモード | 指を離さずカーソルを自由移動。Gboard相当の操作感 |
| 中 | 候補ストリップのスワイプ操作 | 左スワイプで学習削除、右スワイプで最優先登録を1ストロークに |
| 中 | 定型文テンプレート | 頻出文句を候補バーから入力。文体補正と連携して敬語版も自動生成 |
| 低 | 時間帯連動テーマ | 朝/昼/夕/夜でキーボードの色温度が自動変化。「木漏れ日」の世界観を深化 |
| 低 | 確定時の葉っぱパーティクル | 変換確定時に小さな葉が舞うマイクロアニメーション（OFF設定可） |
| 低 | 季節の背景プリセット | 春（桜）夏（新緑）秋（紅葉）冬（雪）を季節に応じて自動提案 |
| 低 | 入力統計（WPM・正確率） | 入力速度とミスの傾向を端末内で確認。改善の動機づけ |

特に上位 4 件は、見た目の追加よりも誤入力・移行・片手操作・誤学習という日常的な
離脱理由を直接減らせます。実装時はパスワード欄で学習しないこと、外部へ暗黙に通信
しないこと、保存するデータを明示することを受け入れ条件にします。

「低」の項目は本プロジェクトの「木漏れ日」という世界観を深める体験設計です。
入力という反復作業に季節感と手触りを添え、IMEを開くこと自体が少し楽しくなる
ことを目指します。いずれもオフライン・最小権限の方針を壊しません。
