...
- [x] mozc を NDK ビルドして JNI 経由で呼ぶ (4 ABI)
- [x] フリック入力キーボード (かな / 英数 / 記号・数字の 3 プレーン)
- [x] Gboard 準拠のキー配置と Material You (端末の壁紙由来の配色) 既定テーマ
- [x] ピュアブラック (有機EL向け)、キーボード背景画像の設定
- [x] 変換候補ストリップ、確定・後退・取り消し・カーソル移動
- [x] 配列とテーマの JSON 外部化 (ユーザー差し替え可)
- [x] 追加辞書を標準同梱 — dic-nico-intersection-pixiv (約 10.2 万語) と
      カタカナ語→英語 (約 3.7 万語、ぶらっく → black)
- [x] ユーザー辞書の GUI 編集 (追加・編集・削除)
- [x] 誤字修正 (フリックの方向ずれ・っ の脱落・転置)
- [x] 設定画面 (IME 有効化、エンジン状態表示、試し入力)
- [ ] 設定 UI からのテーマ/配列編集
- [ ] 変換学習の設定
- [ ] 絵文字・記号パレット
- [ ] マッシュルーム相当の拡張連携
